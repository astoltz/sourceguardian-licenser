    ...
    <div id="{{ $name }}-container">
        @foreach($items as $index => $item)
            <div class="input-group mb-2">
                ...
                <input type="number" class="form-control" name="{{ $name }}[{{ $index }}][order]" value="{{ $item->order }}" placeholder="Order" style="max-width: 100px;">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        addButton.addEventListener('click', function () {
            ...
            row.innerHTML = `
                ...
                <input type="number" class="form-control" name="{{ $name }}[${index}][order]" value="0" placeholder="Order" style="max-width: 100px;">
                <button type="button" class="btn btn-danger remove-row">Remove</button>
            `;
